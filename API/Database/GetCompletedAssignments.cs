...
namespace API.Database
{
    public class GetCompletedAssignments : IReadAllAssignments
    {
        public List<Assignment> GetAssignments(int id)
        {
            ...
            cmd.Connection = con;
            cmd.CommandText = @"SELECT Assign_ID, IsComplete, IsManagerApproved, AssignStatus, DATE_FORMAT(DueDate, '%m-%d-%Y') AS DueDate, DATE_FORMAT(StatusDate, '%m-%d-%Y') AS StatusDate, AssignTitle FROM assignment WHERE AssignedToEmp_ID = @id and IsManagerApproved = 1";
