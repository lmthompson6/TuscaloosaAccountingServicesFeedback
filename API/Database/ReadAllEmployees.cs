...
namespace API.Database
{
    public class ReadAllEmployees : IReadAllEmployees, IReadEmployee
    {
        public List<Employee> GetAllEmployees()
        {
            ...
            while (rdr.Read())
            {
                Employee temp = new Employee() { Emp_ID = rdr.GetInt16(0), EmailAddress = rdr.GetString(1), IsManager = rdr.GetBoolean(2), IsActive = rdr.GetBoolean(3), FirstName = rdr.GetString(4), LastName = rdr.GetString(5), PasswordHash = rdr.GetString(6) };
