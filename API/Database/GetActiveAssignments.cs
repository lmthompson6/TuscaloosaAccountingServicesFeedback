...
namespace API.Database
{
    public class GetActiveAssignments : IReadAllAssignments
    {
        public List<Assignment> GetAssignments(int id)
        {
            ...
            cmd.Connection = con;
            cmd.CommandText = @"SELECT a.Assign_ID, a.IsComplete, a.IsManagerApproved, a.AssignStatus, DATE_FORMAT(a.DueDate, '%m-%d-%Y %r') AS DueDate, DATE_FORMAT(a.StatusDate, '%m-%d-%Y %r') AS StatusDate, a.AssignTitle, concat(e.FirstName, ' ',e.LastName) as name, a.Assess_ID, a.IsReviewing FROM 
